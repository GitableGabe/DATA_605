...
For $f(0),f'(0) \ and \ f''(0)$ the answer is undefined. The best approach is to rework the function to account for a value that is close to but not 0.

I will hence leverage the original taylor series to essentially re-center the Expansion and derive: $f(a)=\sqrt{a}, \ f'(a)=\frac{1}{2\sqrt{a}}, \ f''(a)=-\frac{1}{4a^{3/2}}$

which makes the Taylor Series Expansion $f(x)\approx \sqrt{a}+\frac{1}{2\sqrt{a}}(x-a)-\frac{1}{8a^{3/2}}(x-a)^2$
...
exact_sqrt_q4_p2 <- function(x) {
  sqrt(x)
}

# Define the recentered Taylor expansion
taylor_expansion_recentered_q4_p2 <- function(x, a) {
  f_a <- sqrt(a)
  f_prime_a <- 1 / (2 * sqrt(a))
  f_double_prime_a <- -1 / (4 * a^(3/2))
  f_a + f_prime_a * (x - a) + (f_double_prime_a / 2) * (x - a)^2
}

# Input values for x
x_values_q4_p2 <- c(0.01, 0.05, 0.1, 0.2, 0.5)

# Choose a recentering point
a <- 0.1

# Calculate exact values and recentered approximation
exact_values_q4_p2 <- sapply(x_values_q4_p2, exact_sqrt_q4_p2)
recentered_values_q4_p2 <- sapply(x_values_q4_p2, taylor_expansion_recentered_q4_p2, a = a)

# Create a data frame for the results
results_q4_p2 <- data.frame(
  x = x_values_q4_p2,
  Exact_Value = exact_values_q4_p2,
  Recentered_Approximation = recentered_values_q4_p2
)
